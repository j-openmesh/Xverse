...
function ___xvs_command_log() {
  local LEVEL="${1}"
  local MESSAGE="${2}"
  echo "XVS_COMMAND[${___XVS_COMMAND_FQCN}][...->${_XVS_COMMAND_STATE_TRANSITION_NAME_PREVIOUS}->[${_XVS_COMMAND_STATE_NAME_CURRENT}]->${_XVS_COMMAND_STATE_TRANSITION_NAME_NEXT}]:[${LEVEL}] ${MESSAGE}" >&2
}
